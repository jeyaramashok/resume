...
I am a software engineer at IBM where I build cloud native applications, kubernetes operators and developer tools.
...
\resumeItem{CloudPak Engineering}
{
    \begin{itemize}[leftmargin=*]
        \item Developed Kubernetes controller (using Go) for Redis database, used as shared dependency across IBM CloudPaks.
        \item Developed general-purpose service broker operator for kubernetes (using OSB API).
        \item Developed solution and tooling for disconnected installs of IBM CloudPak on Openshift clusters.
        \item Developed internal developer tools (CLI implemented in Go) for packaging and delivering IBM CloudPaks.
        \item Developed certification framework for delivery of IBM CloudPaks on RedHat OpenShift (RHOCP).
        \item Implemented CICD pipeline using gitops principles, that helped to reduce manual process and promotion time.
        \item Implemented code \& image signing for CICD pipeline, to ensure integrity and avoid supply chain attacks.
        \item Helped develop certification framework containing security best practices and patterns that IBM products must follow when developing production grade applications on Kubernetes.
        \item Helped onboard ISV and opensource helm charts into IBM catalog by addressing security gaps and hardening things to help them meet IBM standards for Kubernetes software.
        \item Responsible for maintaining inner source components.
        \item Participated and contributed in external open source communities like operator-sdk, operator-lifecycle-manager.
    \end{itemize}
}
\resumeItem{Watson Natural Language Understanding}
{
    \begin{itemize}[leftmargin=*]
        ...
        \item Integrated sentiment service with existing stack (Typescript).
...
{
    \begin{itemize}[leftmargin=*]
        \item Distributed Information Systems (ISM 6225) and Information Security and Risk Management (ISM 6328)
    \end{itemize}
}
...
{
    \begin{itemize}[leftmargin=*]
        \item Responsible for profiling JVM and heap dump analysis, to identify potential memory leaks and slow running code.
